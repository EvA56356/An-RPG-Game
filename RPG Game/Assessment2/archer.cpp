...
archer::archer(int lv_in, string name_in)
{
	...
	HPmax = 140 + 8 * (LV - 1);		// HP increases 8 point2 per level
	HP = HPmax;
	MPmax = 85 + 2 * (LV - 1);		// MP increases 2 points per level
	MP = MPmax;
	AP = 30 + 4 * (LV - 1);			// AP increases 4 points per level
	DP = 23 + 4 * (LV - 1);			// DP increases 4 points per level
	speed = 35 + 2 * (LV - 1);		// speed increases 2 points per level
...
void archer::isLevelUp()
{
	if (EXP >= LV * LV * 75)
	{
		...
		cout << "DP improved 4 points to " << DP << endl;
		system("pause");
...
bool archer::attack(player& p)
{
	...
	{
		HPtemp = (int)((1.0 * AP / p.DP) * AP * 5 / (rand() % 3 + 11));		// opponent's HP decrement calculated based their AP/DP, and uncertain chance
	...
	// Normal attack
	HPtemp = (int)((1.35 * AP / p.DP) * AP * 5 / (rand() % 4 + 9));
	cout << name << " uses double shooting, " << p.name << "'s HP decreases " << HPtemp << endl;
...
bool archer::specialatt(player& p)
{
	...
	else
	{
		...
		cout << name << " uses shaped charge shooting, " << p.name << "'s HP decreases " << HPtemp << endl;
		cout << name << " obtained " << EXPtemp << " experience." << endl;
		p.HP = (int)(p.HP - HPtemp);
		EXP = (int)(EXP + EXPtemp);
		system("pause");
