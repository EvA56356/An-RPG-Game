...
mage::mage(int lv_in, string name_in)
{
	...
	HPmax = 120 + 8 * (LV - 1);		// HP increases 8 point2 per level
	HP = HPmax;
	MPmax = 95 + 2 * (LV - 1);		// MP increases 2 points per level
	MP = MPmax;
	AP = 27 + 4 * (LV - 1);			// AP increases 4 points per level
	DP = 23 + 4 * (LV - 1);			// DP increases 4 points per level
	speed = 23 + 2 * (LV - 1);		// speed increases 2 points per level
...
bool mage::specialatt(player& p)
{
	...
	else
	{
		...
		cout << name << " uses Explosion, " << p.name << "'s HP decreases " << HPtemp << endl;
		cout << name << " obtained " << EXPtemp << " experience." << endl;
		p.HP = (int)(p.HP - HPtemp);
		EXP = (int)(EXP + EXPtemp);
		system("pause");
